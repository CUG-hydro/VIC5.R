...
#' @param val Value of the parameter to be set. Will return the current setting 
#' value when not provided.
#' 
#' @details The global paremteres are as follows:
#'
#'  \describe{
#'
#'   \item{nlayers}{Integer. Number of moisture layers used by the model.
#'   Default = 3.} \item{nnodes}{Integer. Number of thermal solution nodes in
#'   the soil column.} \item{step_per_day}{Integer. Number of simulation time
#'   steps per day. Should be > 4 when full_energy=TRUE or frozen_soil=TRUE.}
#'   \item{snow_step_per_day}{Integer. Number of time steps per day used to
#'   solve the snow model (if step_per_day > 1, this should = step_per_day)}
#'   \item{runoff_step_per_day}{Integer. Number of time steps per day used to
#'   solve the runoff model (should be >= step_per_day)}
...
#'
#'   \item{start_sec}{Integer. Second of model simulation starts (e.g.,
#'   start_sec = 0 means starting from the beginning of a day; start_sec = 3600
#'   for starting from the beginning of the second hour of a day). Default = 0.}
#'
#'   \item{nrecs}{Integer. Number of time steps over which to run model. The
...
#'
#'   \item{calendar}{Boolean. Calendar to use. Calendar to use. Must be one of:
#'   "standard", "gregorian", "proleptic_gregorian", "noleap", "365_day",
#'   "360_day", "julian", "all_leap", "366_day."}
#'
#'   \item{time_units}{Boolean. Units for output time variables. Units for
#'   output time variables. Should be one of "days", "hours", "minutes",
#'   "seconds". Default = days.}
#'
#'   \item{full_energy}{Boolean. Option for computing land surface temperature
...
#'   equal to air temperature. Default = False.}
#'
#'   \item{close_energy}{Boolean. Option for controlling links between the
...
#'   step.?? Default = FALSE.}
#'
#'   \item{frozen_soil}{Boolean. Option for handling the water/ice phase change
...
#'   the soil parameters (parameter `soil` of [vic()]) for each grid cell where
...
#'   areas) to skip the extra computation.}
#'
#'   \item{quick_flux}{Boolean. Option for computing the soil vertical
...
#'   et al. ,1999) in all other cases.}
#'
#'   \item{implicit}{Boolean. If TRUE the model will use an implicit solution
...
#'   an error message. Default = TRUE.}
#'
#'   \item{quick_solve}{Boolean. This option is a hybrid of quick_flux. If TRUE,
...
#'   final solution step. Default = FALSE.}
#'
#'   \item{no_flux}{Boolean. If TRUE model will use a no flux bottom boundary
...
#'   constant temperature bottom boundary condition).}
#'
#'   \item{exp_trans}{Boolean. If TRUE the model will exponentially distributes
...
#'   frozen_soil = TRUE). Default = TRUE.}
#'
#'   \item{grnd_flux_type}{Integer. Options for handling ground flux: 0 = use
...
#'   releases. Default = 1.}
#'
#'   \item{Tfallback}{Boolean. Options for handling failures of T iterations to
...
#'   node T solution (one time series per node). Default = TRUE.}
#'
#'   \item{share_layer_moist}{Boolean. If TRUE, then??if??the soil moisture in
...
#'   moisture. Default: TRUE.}
#'
#'   \item{spatial_frost}{Integer. Option to allow spatial heterogeneity in soil
...
#'   temperatures will be set to mean temperature +/- frost_slope. Default = 0.}
#'
#'   \item{snow_den}{Boolean. Options for computing snow density. 0 = Use
#'   algorithm taken from SNTHRM model.1 = Use traditional VIC algorithm taken
#'   from Bras (1990). Default = 0.}
#'
#'   \item{blowing}{Boolean. If TRUE, compute evaporative fluxes due to blowing
#'   snow. Default = FALSE.}
#'
#'   \item{blowing_var_threshold}{Boolean. If TRUE, a variable shear stress
...
#'   TRUE.}
#'
#'   \item{blowing_calc_prob}{Boolean. If TRUE, the probability of occurrence of
...
#'
#'   \item{blowing_simple}{Boolean. If TRUE, the sublimation flux of blowing
...
#'   suspension layer. See Lu and Pomeroy (1997) for details. Default: FALSE.}
#'
#'   \item{blowing_fetch}{Boolean. This option is only used when BLOWING_SIMPLE
...
#'   TRUE.}
#'
#'   \item{blowing_spatial_wind}{Boolean. If TRUE, multiple wind speed ranges,
...
#'   is used. See Lu and Pomeroy (1997) for details. Default: TRUE.}
#'
#'   \item{compute_treeline}{Integer. Options for handling above-treeline
...
#'   parameters (parameter `soil` of [vic()]), and set the july_tavg option to
#'   TRUE so that VIC can read the soil parameters correctly. If lakes=TRUE,
#'   compute_treeline must be FALSE. Default = FALSE.}
#'
#'   \item{corrprec}{Boolean. If TRUE correct precipitation for gauge
#'   undercatch. This option is not supported when using snow/elevation bands.
#'   Default = FALSE.}
#'
#'   \item{spatial_snow}{Boolean. Option to allow spatial heterogeneity in snow
...
#'   extra field in the soil parameters (parameter `soil` of [vic()]).
#'   max_snow_distrib_slope should be set to twice the desired minimum spatial
#'   average snow pack depth `[m]`. I.e., if we define depth_thresh to be the
...
#'   recovered. Default = FALSE.}
#'
#'   \item{AERO_resist_cansnow}{Integer. Options for aerodynamic resistance in
...
#'   2.}
#'
#'   \item{carbon}{Boolean. Options for simulating carbon cycle or not. FALSE??=
#'   do not simulate carbon cycle. TRUE??= simulate carbon cycle.??Default =
#'   FALSE.}
#'
#'   \item{RC_mode}{Integer. Determines how canopy resistance is computed. 0??=
...
#'   moisture limitation).?? Default = 0.}
#'
#'   \item{veglib_photo}{Boolean. Tells VIC about the contents of the veg
...
#'
#'   \item{continue_error}{Boolean. Options for handling fatal errors. FALSE??=
...
#'   cell.??Default = TRUE.}
#'
#'   \item{wind_h}{Numeric. Height `[m]` of wind speed measurement over bare soil
...
#'   vegetation canopy is not defined.}
#'
#'   \item{canopy_layers}{Integer. Number of canopy layers in the model.
#'   Default: 3.}
#'
#'   \item{baseflow}{Integer. This option describes the form of the baseflow
#'   parameters in the soil parameters (parameter `soil` of [vic()]): 0 = fields
...
#'   et al (2001) Default = 0.}
#'
#'   \item{july_tavg}{Boolean. If TRUE then VIC will expect an additional column
#'   (July_Tavg) in the soil parameters (parameter `soil` of [vic()]) to contain
...
#'   Default = FALSE.}
#'
#'   \item{organic}{Boolean. TRUE = the soil parameters (parameter `soil` of
#'   [vic()]) contains 3*Nlayer extra columns. For each layer: the organic
...
#'   be 0. Default = FALSE.}
#'
#'   \item{nrootzones}{Integer. Number of defined root zones defined for root
#'   distribution.}
#'
#'   \item{vegpar_albedo}{Boolean. If TRUE the vegetation parameters (parameter
#'   `veg` of [vic()]) contains the columns for each vegetation type that
#'   defines monthly albedo values for each vegetation type for each grid cell.
#'   Default = FALSE.}
#'
#'   \item{albedo_src}{Integer. This option tells VIC where to look for ALBEDO
#'   values: 1 = Use the ALBEDO values listed in the vegetation library. 2 = Use
#'   the ALBEDO values listed in the vegetation parameters (parameter `veg` of
#'   [vic()]). Note: for this to work, vegpar_albedo must be TRUE. 3= Use the
#'   albedo values in the parameter `forcing_veg` of [vic()]. For this to work,
#'   albedo must be supplied in code{forcing_veg}. Default = 1.}
#'
#'   \item{vegpar_LAI}{Boolean. If TRUE the vegetation parameters (parameter
#'   `veg` of [vic()]) contains an extra line for each vegetation type that
#'   defines monthly LAI values for each vegetation type for each gridcell.
#'   Default = FALSE.}
#'
#'   \item{LAI_src}{Boolean. This option tells VIC where to look for LAI values:
#'   1 = Use the LAI values listed in the vegetation library. 2 = Use the LAI
#'   values listed in the vegetation parameters (parameter `veg` of [vic()]).
#'   Note: for this to work, vegpar_LAI must be TRUE. 3= Use the LAI values in
#'   the parameter `forcing_veg` of [vic()]. For this to work, albedo must be
#'   supplied in code{forcing_veg}. Default = 1.}
#'
#'   \item{veglib_fcan}{Boolean. If TRUE the vegetation library contains monthly
#'   FCANOPY values for each vegetation type for each grid cell (between the LAI
#'   and ALBEDO values). Default = FALSE.}
#'
#'   \item{vegpar_fcan}{Boolean. If TRUE the vegetation parameters (parameter
#'   `veg` of [vic()]) contains an extra line for each vegetation type that
#'   defines monthly FCANOPY values for each vegetation type for each grid cell.
#'   Default = FALSE.}
#'
#'   \item{fcan_src}{Boolean. This option tells VIC where to look for FCANOPY
...
#'   in the parameter `forcing_veg` of [vic()]. For this to work, FCANOPY must
#'   be supplied in code{forcing_veg} Default = 0.}
#'
#'   \item{nbands}{Integer. Maximum number of snow elevation bands to use.
#'   Parameter snowband should be provided for the [vic()] function when > 1.
#'   Default = 1.}
#'
#'   \item{lakes}{Boolean. Options for if simulate lakes lake parameter or not.
#'   Default = FALSE.}
#'
#'   \item{lake_profile}{Boolean. Options for describing lake profile: FALSE??=
...
#'   FALSE.}
#'
#'   \item{resolution}{Numeric. Width of grid cells, in decimal degree latitude
...
#' Addison-Wesley. 
#'
#' Cherkauer, K. A. and D. P. Lettenmaier, 1999: Hydrologic effects of frozen
#' soils in the upper Mississippi River basin, J. Geophys. Res., 104(D16),
#' 19,599-19,610. 
#'
#' Liang, X., E. F. Wood, and D. P. Lettenmaier, 1999: Modeling ground heat flux
