...
void get_options(List vic_options) {
  ...
  for (int i = 0; i < nopts; i++) {
     ...
     * ***********************************************************************/
      ...
      if (intopt == 3) {
        ...
        log_err(
            "Unrecognized value of LAI_src in the global "
            "options.");
      }
      ...
      if (intopt == 3) {
        ...
        log_err(
            "Unrecognized value of fcan_src in the global "
            "options.");
      }
      ...
      if (intopt == 3) {
        ...
        log_err(
            "Unrecognized value of ALB_SRC in the global "
            "options.");
      }
  ...
  if (global_param.model_steps_per_day == 0) {
    log_err(
        "Model time steps per day has not been defined.  Make sure "
    ...
    log_err(
        "The specified number of model steps per day (%zu) > 1 and < "
    ...
    log_err(
        "The specified number of model steps per day (%zu) > the "
    ...
    log_err(
        "The specified number of model steps per day (%zu) is > 24 "
  ...
  if (global_param.snow_steps_per_day == 0) {
    log_err(
        "Snow model time steps per day has not been defined.  Make "
    ...
    log_err(
        "The specified number of snow model steps per day (%zu) < "
        "the minimum number of subdaily steps per day (%d).  Make "
    ...
    log_err(
        "The specified number of snow steps per day (%zu) > the "
        "the maximum number of subdaily steps per day (%d).  Make "
        "sure that the global options defines SNOW_STEPS_PER_DAY of at "
        "most (%d).",
        global_param.snow_steps_per_day,
    ...
    log_err(
        "The specified number of snow model steps per day (%zu) is > "
        ...
        global_param.snow_steps_per_day);
             ...
             0) {
    log_err(
        "The specified number of snow model timesteps (%zu) must be "
  ...
  if (global_param.runoff_steps_per_day == 0) {
    log_err(
        "Runoff time steps per day has not been defined.  Make "
        "sure that the global options defines RUNOFF_STEPS_PER_DAY.");
  } else if (global_param.runoff_steps_per_day <
             MIN_SUBDAILY_STEPS_PER_DAY) {
    log_err(
        "The specified number of runoff steps per day (%zu) < "
        "the minimum number of subdaily steps per day (%d).  Make "
    ...
    log_err(
        "The specified number of runoff steps per day (%zu) is > "
        "24 and is not evenly divided by 24.",
        global_param.runoff_steps_per_day);
  } else if (global_param.runoff_steps_per_day >
             MAX_SUBDAILY_STEPS_PER_DAY) {
    log_err(
        "The specified number of runoff steps per day (%zu) > the "
        "the maximum number of subdaily steps per day (%d).  Make "
        "sure that the global options defines RUNOFF_STEPS_PER_DAY of at "
        "most (%d).",
        global_param.runoff_steps_per_day,
        ...
        MAX_SUBDAILY_STEPS_PER_DAY);
             ...
             0) {
    log_err(
        "The specified number of runoff timesteps (%zu) must be "
        "evenly divisible by the number of model timesteps per day "
        "(%zu)",
        global_param.runoff_steps_per_day,
        global_param.model_steps_per_day);
  ...
  if (global_param.atmos_steps_per_day < MIN_SUBDAILY_STEPS_PER_DAY) {
    log_err(
        "The specified number of atmos steps per day (%zu) < "
        "the minimum number of subdaily steps per day (%d).  Make "
    ...
    log_err(
        "The specified number of atmos steps per day (%zu) is > "
        "24 and is not evenly divided by 24.",
        global_param.atmos_steps_per_day);
  } else if (global_param.atmos_steps_per_day > MAX_SUBDAILY_STEPS_PER_DAY) {
    log_err(
        "The specified number of atmos timesteps per day (%zu) > the "
        "the maximum number of subdaily steps per day (%d).  Make "
        "sure that the global options defines ATMOS_STEPS_PER_DAY of at "
        "most (%d).",
        global_param.atmos_steps_per_day,
        ...
        MAX_SUBDAILY_STEPS_PER_DAY);
           ...
           0) {
    log_err(
        "The specified number of atmos timesteps (%zu) must be "
        "evenly divisible by the number of model timesteps per day "
        "(%zu)",
        global_param.atmos_steps_per_day,
        global_param.model_steps_per_day);
             ...
             0) {
    log_err(
        "The specified number of atmos timesteps (%zu) must be evenly "
  ...
  if (global_param.startmonth == 0) {
    ...
    log_err(
        "The specified simulation start month (%hu) > 12. Make "
  ...
  if (global_param.nrecs == 0 && global_param.endyear == 0 &&
    ...
    log_err(
        "The model global options MUST define EITHER the number of "
    ...
    log_err(
        "The specified duration of simulation (%zu) < 1 time step. "
  ...
  *******************************************************************************/
  ...
  if (options.LAI_SRC == FROM_VEGPARAM && !options.VEGPARAM_LAI) {
    log_err(
        "\"LAI_SRC\" was specified as \"FROM_VEGPARAM\", but "
  ...
  if (options.ALB_SRC == FROM_VEGPARAM && !options.VEGPARAM_ALB) {
    log_err(
        "\"ALB_SRC\" was specified as \"FROM_VEGPARAM\", but "
  ...
  if (options.FCAN_SRC == FROM_VEGPARAM && !options.VEGPARAM_FCAN) {
    log_err(
        "\"FCAN_SRC\" was specified as \"FROM_VEGPARAM\", but "
  ...
  if (options.FCAN_SRC == FROM_VEGLIB && !options.VEGLIB_FCAN) {
    log_err(
        "\"FCAN_SRC\" was specified as \"FROM_VEGLIB\", but "
  ...
  if (!options.CARBON) {
    if (options.RC_MODE == RC_PHOTO) {
      log_warn(
          "If CARBON==FALSE, RC_MODE must be set to "
  ...
  if (options.QUICK_FLUX) {
    if (options.Nnode != 3) {
      log_warn(
          "To run the model QUICK_FLUX=TRUE, you must "
  ...
  if (options.QUICK_SOLVE && !options.QUICK_FLUX) {
    if (options.NOFLUX) {
      log_err(
          "NOFLUX must be set to FALSE when QUICK_SOLVE=TRUE "
          "and QUICK_FLUX=FALSE");
    }
    if (options.EXP_TRANS) {
      log_err(
          "exp_trans must be set to FALSE when QUICK_SOLVE=TRUE "
          "and QUICK_FLUX=FALSE");
    }
  }
  ...
  if (options.Nlayer > MAX_LAYERS) {
    log_err(
        "Global options wants more soil moisture layers (%zu) than "
  ...
  if (options.Nnode > MAX_NODES) {
    log_err(
        "Global options wants more soil thermal nodes (%zu) than are "
  ...
  if (options.LAKES) {
    ...
    if (global_param.resolution == 0) {
      log_err(
          "The model grid cell resolution must be "
