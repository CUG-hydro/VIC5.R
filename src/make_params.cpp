  ...
  **********************************************/
  for (layer = 0; layer < options.Nlayer; layer++) {
    if (temp->depth[layer] < MINSOILDEPTH) {
      log_err(
          "Model will not function with layer %zu "
          "depth %f < %f m.",
  ...
  if (temp->depth[0] > temp->depth[1]) {
    log_err(
        "Model will not function with layer %d depth"
  ...
  for (layer = 0; layer < options.Nlayer; layer++) {
    ...
    if (temp->Wpwp[layer] < temp->resid_moist[layer] *
      ...
      log_err(
          ...
          temp->Wpwp[layer], temp->resid_moist[layer] * temp->depth[layer] * MM_PER_M, layer);
    }
  }
    ...
    if (!options.EXP_TRANS) {
      ...
      if (temp->Zsum_node[0] > temp->depth[0]) {
        log_err(
            ...
            "dp (%f)",
            temp->Zsum_node[0], temp->depth[0], dp);
      }
    ...
    if (zwt_prime == 0) {
      ...
      while (layer > 0) {
        ...
        zwt_prime_eff = tmp_depth2_save * CM_PER_M - bubble +
                        bubble *
                            pow((zwt_prime + bub_save - tmp_depth2_save * CM_PER_M) / bub_save, b / b_save);
        w_avg = -(b / (b - 1)) * bubble *
                (1 - pow((zwt_prime_eff + bubble - tmp_depth2 * CM_PER_M) / bubble,
                         (b - 1) / b)) /
                (temp->depth[layer] * CM_PER_M);
        tmp_moist += w_avg * (temp->max_moist[layer] -
                              tmp_resid_moist) +
  ...
  for (i = 0; i < Nveg_type; i++) {
    ...
    for (j = 0; j < NM; j++) {
      ...
      if (options.LAI_SRC == FROM_VEGLIB && temp[i].overstory &&
          temp[i].LAI[j] == 0) {
        log_err(
            "veg library: the specified veg class (%d) "
...
make_vegparam(NumericMatrix veg_par,
  ...
  for (i = 0; i < vegetat_type_num; i++) {
    ...
    if (sum != 1.) {
      log_warn(
          "Root zone fractions of cell %i sum to more than 1 ( = %f), "
    ...
    if (options.BLOWING) {
      ...
      if (temp[i].fetch < 1.0) {
        log_err(
            "BLOWING parameter fetch should be >> 1 but "
            "cell %i has fetch = %.2f",
    ...
    if (options.VEGPARAM_LAI && options.LAI_SRC == FROM_VEGPARAM) {
      for (j = 0; j < MONTHS_PER_YEAR; j++) {
        ...
        if (veg_lib[temp[i].veg_class].overstory &&
            temp[i].LAI[j] == 0) {
          log_err(
              "cell %d, veg tile %d: the specified "
  ...
  if (Cv_sum > 1.0) {
    log_warn(
        "Cv_sum exceeds 1.0 (%f) at grid cell %d, fractions being "
        "adjusted to equal 1",
        Cv_sum, gridcel);
    ...
    log_warn(
        "Cv > 0.99 and Cv < 1.0 at grid cell %d, model "
...
void make_snowband(NumericVector snowband, soil_con_struct *soil_con) {
  ...
  if (Nbands > 1) {
    ...
    if (total != 1.) {
      log_warn(
          "Sum of the snow band area fractions of cell %i does not "
    ...
    if (fabs(avg_elev - soil_con->elevation) > 1.0) {
      log_warn(
          "average band elevation %f not equal to grid_cell "
    ...
    if (total != 1.) {
      log_warn(
          "Sum of the snow band precipitation fractions "
...
lake_con_struct make_lakeparam(NumericVector lake_par,
  ...
  if (temp.lake_idx >= 0) {
    ...
    if (temp.numnod > MAX_LAKE_NODES) {
      log_err(
          ...
          "edit MAX_LAKE_NODES in user_def.h.",
          temp.numnod,
    ...
    if (temp.mindepth < 0) {
      log_err(
          ...
          "least 0.",
          temp.mindepth, soil_con.gridcel);
    }
    ...
    if (temp.wfrac < 0 || temp.wfrac > 1) {
      log_err(
          ...
          "Change wfrac to be between 0 and 1.",
          temp.wfrac,
          soil_con.gridcel);
    }
    ...
    if (temp.depth_in < 0) {
      log_err(
          ...
          "lake parameter is < 0; increase this number to at "
          "least 1.",
          temp.depth_in, soil_con.gridcel);
    }
  ...
  if (!options.LAKE_PROFILE) {
    ...
    if (fabs(1 - temp.Cl[0] / veg_con[temp.lake_idx].Cv) > 0.01) {
      log_err(
          "Lake area fraction at top of lake basin (%f) for cell "
    ...
    for (i = 0; i < temp.numnod; i++) {
      ...
      if (i == 0) {
        if (fabs(1 - temp.Cl[0] / veg_con[temp.lake_idx].Cv) > 0.01) {
          log_err(
              "Lake area fraction at top of lake basin (%f) "
      ...
      if (temp.Cl[i] < 0.0 || temp.Cl[i] > 1.0) {
        log_err(
            "Lake layer %d area fraction (%f) for cell (%d) "
  ...
  if (temp.mindepth > temp.maxdepth) {
    log_err(
        ...
        "lake depth %f.",
        temp.mindepth, temp.maxdepth);
  }

  // Validate initial conditions
  if (temp.depth_in > temp.maxdepth) {
    log_warn(
        "Initial lake depth %f exceeds the maximum lake depth %f; "
