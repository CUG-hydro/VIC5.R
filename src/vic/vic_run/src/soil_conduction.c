...
soil_conductivity(double moist,
  ...
  Kdry_min = (0.135 * bulk_dens_min + 64.7) /
      (soil_dens_min - 0.947 * bulk_dens_min);
 ...
 ******************************************************************************/
void set_node_parameters(double *Zsum_node,
 ...
 ******************************************************************************/
void estimate_frost_temperature_and_depth(double ***tmpT,
                                          double **tmpZ,
                                          double *Zsum_node,
                                          double *T,
                                          double *depth,
  ...
  for (lidx = 0; lidx < Nlayers; lidx++) {
    ...
    if (max_nidx >= Nnodes) {
      log_warn(
          "Soil thermal nodes do not extend below bottom "
 ...
 ******************************************************************************/
int estimate_layer_temperature(layer_data_struct *layer,
                               double ***tmpT,
                               double **tmpZ,
                               double *Zsum_node,
                               double *depth,
 ...
 *           is TRUE.
 ******************************************************************************/
int estimate_layer_temperature_quick_flux(layer_data_struct *layer,
                                          double *depth,
                                          double Dp,
                                          double Tsurf,
                                          double T1,
...
maximum_unfrozen_water(double T,
  ...
  if (T < 0.) {
    unfrozen = max_moist *
               pow((-CONST_LATICE * T) /
                       (CONST_TKTRIP) / (CONST_G * bubble / (CM_PER_M)),
                   -(2.0 / (expt - 3.0)));
